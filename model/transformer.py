...
from model.Transformer_layer import Transformer_layer
from einops import rearrange

def get_time_embedding(time_steps, temp_dim):
    r"""
    ...
    """
    assert temp_dim % 2 == 0, "time embedding dimension must be divisible by 2"
    # factor = 10000^(2i/d_model)
    factor = 10000 ** ((torch.arange(
        start=0,
        end = temp_dim // 2,
        dtype=torch.float32,
        device=time_steps.device) / (temp_dim // 2)
        ))
    # pos / factor
    # timesteps B -> B, 1 -> B, temp_dim
    t_emb = time_steps[:,None].repeat(1,temp_dim//2)/factor
...
class DIT(nn.Module):
    def __init__(self, im_size, im_channels, config):
        ...
        self.patch_embed_layer = PatchEmbed(image_height=self.image_height, 
                                            image_width=self.image_width,
                                            im_channels=self.im_channel,
        ...
        self.layers = nn.ModuleList([
            Transformer_layer(config) for _ in range(num_layers)
        ])
        ...
        nn.init.constant_(self.proj_out[0].weight, 0) 
    ...
    def forward(self, x, t):
        ...
        pre_mlp_shift, pre_mlp_scale = self.adaptive_norm_layer(t_emb).chuck(2, dim=1)
        out = (self.norm(out) * (1 + pre_mlp_scale.unsqueeze(1)) + pre_mlp_shift.unsqueeze(1))
        # Unpatchify
        out = self.proj_out(out)
        out = rearrange(out, 'b (nh nw) (ph pw c) -> b c (nh ph)(nw pw)',ph=self.patch_height,
                        pw=self.patch_width,nw=self.nw,nh = self.nh)
        return out
